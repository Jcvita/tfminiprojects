"""
    ...
    Learns to Play Breakout
    analyses frame by frame to determine how it's doing

    Q(s, a) tries to determine what Action (a) is statistically the best choice based on the State (s)
    and the rewards given by the action (updates Q accordingly). 
"""
import os
...
class DeepQNetwork(object):
    ...
    def build_net(self):
        with tf.variable_scope(self.name):
            ...
            self.loss = tf.reduce_mean(tf.square(self.q - self.q_targets))
    ...
    class Agent(object):
        """
            The 'Player'. Contains data for a States and Actions
            State: the current state of the Agent (observable game information and learned knowlege)
            Action: all available actions the Agent can perform
        """
        def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size, replace_target=5000,
                    input_dims=(210,160,4), q_next='tmp/q_next', q_eval='tmp/q_eval'):
            ...
            self.q_next = DeepQNetwork(alpha, n_actions, input_dims=input_dims, name='q_next',
                    chkpt=q_next_dir)
            self.q_eval = DeepQNetwork(alpha, n_actions, input_dims=input_dims, name='q_eval',
